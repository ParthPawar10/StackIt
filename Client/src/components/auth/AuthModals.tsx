...
import { initiateOAuthLogin } from '../../utils/oauth.ts';
...
export default function AuthModals({ isOpen, onClose, initialType }: AuthModalsProps) {
  const [currentType, setCurrentType] = useState(initialType);
  const [showPassword, setShowPassword] = useState(false);
  const [showConfirmPassword, setShowConfirmPassword] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [isOAuthLoading, setIsOAuthLoading] = useState<string | null>(null);
  const { login, register, updateUserFromToken } = useAuth();
  ...
  const handleOAuthLogin = async (provider: 'google' | 'github') => {
    try {
      ...
      const token = await initiateOAuthLogin(provider);
          ...
          <div className="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-medium text-gray-900 dark:text-white">
                {currentType === 'login' ? 'Sign in to StackIt' : 'Join StackIt'}
              </h3>
              <button
                onClick={onClose}
              ...
              <form onSubmit={loginForm.handleSubmit(onLoginSubmit)} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  >
                    ...
                    <span className="ml-2">{isOAuthLoading === 'google' ? 'Loading...' : 'Google'}</span>
                  </button>
                  ...
                  >
                    ...
                    <span className="ml-2">{isOAuthLoading === 'github' ? 'Loading...' : 'GitHub'}</span>
                  </button>
                </div>
